...
import { editAuthor } from '../authors/authorsSlice'
import type { IAuthorState } from './author.types'
import { editBooksByAuthorId } from '../books/booksSlice'

export default function EditAuthorPage() {
	const params = useParams()
	const authorId = params.id
	const author = useSelector((state: IAuthorState) =>
		state.authors.find(author => author.id === authorId)
	)

	const [name, setName] = useState<string>(author ? author.name : '')
	const [description, setDescription] = useState<string>(author ? author.description : '')
	...
	const onSaveAuthor = () => {
		if (name && description) {
			...
			dispatch(editBooksByAuthorId({ authorId: authorId!, authorName: name }))
	...
	return (
		<div>
			<h1 className='text-center text-4xl font-bold text-gray-900'>Редактировать автора</h1>
			<form>
